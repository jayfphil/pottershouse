 ...
 *
	...
	a: croping align (l, r, b, t)
	f: image filters (1,2,3, etc)
...
define ('DIRECTORY_CACHE', './_cache');          // cache directory
define ('DIRECTORY_TEMP', './_temp');            // temp directory
    ...
    if (file_exists ($cache_file)) {
        ...
        if (isset ($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
            if (strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) < strtotime('now')) {
                ...
                $gmdateModified = gmdate('D, d M Y H:i:s') . ' GMT';

                // send content headers then display image
                header ('Content-Type: ' . $mime_type);
                header ('Accept-Ranges: bytes');
                header ('Last-Modified: ' . $gmdateModified);
