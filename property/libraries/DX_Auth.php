...
class DX_Auth
{
	...
	function _init()
	{
		...
		$this->clerk_home_uri			= $this->ci->config->item('DX_clerk_home_uri');
		$this->director_home_uri		= $this->ci->config->item('DX_director_home_uri');		
		$this->admin_home_uri	        = $this->ci->config->item('DX_admin_home_uri');
		$this->user_uri			        = $this->ci->config->item('DX_user_uri');
		$this->clerk_uri			    = $this->ci->config->item('DX_clerk_uri');
		$this->director_uri		        = $this->ci->config->item('DX_director_uri');		
		$this->admin_uri	            = $this->ci->config->item('DX_admin_uri');
		...
		$this->home_view			 	= $this->ci->config->item('DX_home_view');
	...
	*/
	...
	function _set_last_ip_and_last_login($user_id)
	{
		...
		{
			// Load model
			$this->ci->load->model('super_admin/users', 'users');
			// Update record
	...
	{		
		...
		{
			// Load model
			$this->ci->load->model('super_admin/login_attempts', 'login_attempts');		
	...
	*/
	...
	{
		return $this->ci->session->userdata('DX_companyname');
	}
	...
	function is_clerk()
	{
		return strtolower($this->ci->session->userdata('DX_role_name')) == 'clerk';
	}
	
	// Check is user has super admin privilege
	function is_director()
	{
		return strtolower($this->ci->session->userdata('DX_role_name')) == 'director';
	}
	...
	function login($login, $password, $remember = TRUE)
	{
		...
		if(!empty($login) AND ! empty($password))
		{
			...
			else
			{
				...
				$this->_increase_login_attempt();
				// Set error message
				$this->_auth_error = $this->ci->lang->line('auth_email_not_exist');
			}
		}
	...
	function register($role_id, $name_title, $first_name, $last_name, $password, $email, $user_id)
	{		
		// Load Models
		$this->ci->load->model('admin/users', 'users');
		$this->ci->load->model('admin/user_temp', 'user_temp');
		$this->ci->load->model('admin/name_titles', 'nameTitles');
	    ...
	    $nTitle = $this->ci->nameTitles->getNameInfo($name_title);
          
		  if(empty($nTitle)):
		    $nData = array('name_title' => $name_title);
			$this->ci->nameTitles->saveNameInfo($nData);
		  endif;
