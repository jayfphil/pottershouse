	...
	<tr>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
                ...
                <?php if(count($contactPersonValues)): ?>
				<table border="0" width="100%" cellpadding="0" cellspacing="0"  style="margin-bottom:5px;">
                    <tr>
                     <td colspan="2" align="left">
                     <a href="<?php echo base_url().$this->aadmin->Acontact_person_entry_uri; ?>"><img src="<?php echo base_url(); ?>public/images/add.gif" alt="<?php echo "#"; ?>" border="0" align="absmiddle"/></a>
                ...
                <?php $i = (int) $this->uri->segment(4) + 1; foreach($contactPersonValues as $contactPersonValue): 
				      $company_id = $contactPersonValue->company_id;
					  $companys = $this->Shows->getThisValue("id = '$company_id'", 'client_company');	
				      $person_level_id = $contactPersonValue->person_level_id;
					  $levels = $this->Shows->getThisValue("id = '$person_level_id'", 'person_level');				
				 ?>
                
				<tr <?php if($i %2 == 0): ?> class="alternate-row" <?php endif;?>>
					<td><?php echo form_checkbox('checkbox_'.$contactPersonValue->id, $contactPersonValue->id); ?></td>
					<td><?php echo $i; ?></td>
			        <td><?php if(count($companys)): foreach($companys as $company): echo ucwords($company->company_name); endforeach; endif; ?></td>
			        <td><?php echo ucwords($contactPersonValue->contact_person_name); ?></td>
			        <td><?php echo ucwords($contactPersonValue->designation); ?></td>
                    <td><?php echo $contactPersonValue->contact_no; ?></td>
                    <td><?php echo $contactPersonValue->email; ?></td>
					<td class="options-width">
					<a href="<?php echo base_url().$this->aadmin->Acontact_person_edit_uri.$contactPersonValue->id; ?>" title="Edit" class="icon-1 info-tooltip"></a>
					<a onclick="return confirmSubmit()" href="<?php echo base_url().$this->aadmin->Acontact_person_delete_uri.$contactPersonValue->id; ?>" title="Delete" class="icon-2 info-tooltip"></a>
					</td>
				</tr>
				...
				<!--  end product-table................................... --> 
                 <?php  else: ?>
				<table border="0" width="100%" cellpadding="0" cellspacing="0"  style="margin-bottom:5px;">
				<tr height="40"><div id="message-red"><td colspan="8"  class="red-left">No Contact Person Available Yet .. <a href="<?php echo base_url().$this->aadmin->Acontact_person_entry_uri; ?>">Add Contact Person Now</a></td></div></tr>
				</table>
                <?php endif; ?>
				<?php echo form_close(); ?>
			</div>
